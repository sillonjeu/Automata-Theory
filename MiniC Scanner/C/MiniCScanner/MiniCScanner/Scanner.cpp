...
struct tokenType scanner()
{
	...
	do {
		...
		if (superLetter(ch)) { // identifier or keyword
			...
			if (token.number == tlitdouble) { // if number is double
				ch = fgetc(sourceFile); // . 
				ch = fgetc(sourceFile); 
				col += 2; 
				
				if (!isdigit(ch) && ch != 'e') { // if ch is not digit
					token.value.dnum = (double)token.value.num;
				...
				double test = 0.0;
				if (isdigit(ch)) test = getDoubleNumber(ch);
				token.value.dnum = (double)token.value.num + test;

				if (fgetc(sourceFile) == 'e') {
					ch = fgetc(sourceFile);
					col += 2;
					int size;
					if (ch == '+') {
						ch = fgetc(sourceFile);
						col++;
						size = getNumber(ch, token); 
					}
					else if (isdigit(ch)) {
						size = getNumber(ch, token);
						for (int i = 0; i < size; i++) token.value.dnum *= 10;
					}
					else if (ch == '-') {
						ch = fgetc(sourceFile);
						col++; 
						size = getNumber(ch, token);
						for (int i = 0; i < size; i++) token.value.dnum *= 0.1; 
					}
					else {
						lexicalError(5);
						ungetc(ch, sourceFile);
...
void lexicalError(int n)
{
	...
	case 4: printf("invalid character\n");
		break;
	case 5: printf("please input number\n");
		break;
